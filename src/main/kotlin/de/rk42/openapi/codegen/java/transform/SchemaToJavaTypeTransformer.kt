...
class SchemaToJavaTypeTransformer(private val configuration: CliConfiguration) {
  ...
  private val uniqueNameConverter = UniqueNameConverter()
   ...
   */
  fun toJavaType(schema: CtrSchemaNonRef): JavaAnyType = schemaToTypeLookup.getOrPut(schema) {
    when (schema) {
  ...
  private fun toGeneratedJavaType(schema: CtrSchemaNonRef, isEnum: Boolean): JavaType {
    val typeName = uniqueNameConverter.toUniqueTypeName(suggestName(schema))
  ...
  private fun suggestName(schema: CtrSchemaNonRef) = schema.title ?: schema.nameHint.path.joinToString("/")
